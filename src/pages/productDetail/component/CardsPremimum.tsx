...
const CardsPremimum = () => {
  ...
  const [product, setProduct] = useState<any>({title: cardData.title, logo: imageUrl, img: "", price: "200", type: ""})
  // const handleChangeUrl = (e) => fileReader.readAsDataURL(e.target.files[0])

  const handleChange = (e:any) => {
    setProduct({...product, [e.target.name]: e.target.name === "title" ? e.target.value.toUpperCase() : product[e.target.name]})
